...
namespace ConstructionLine.CodingChallenge.Tests
{
    public class SearchEngineTestsBase
    {
        ...
        protected static IEnumerable<ColorCount> GenerateExpectedColourCountList(
            IEnumerable<(Color Colour, int Count)> overrideColourCounts = null)
        {
            var expectedColourCounts = Color.All.Select(c => new ColorCount() { Color = c, Count = 0 }).ToList();
            if (overrideColourCounts != null)
            {
                foreach (var (colour, count) in overrideColourCounts)
                {
                    expectedColourCounts.First(c => c.Color == colour).Count = count;
                }
            }


            return expectedColourCounts;
        }

        protected static IEnumerable<SizeCount> GenerateExpectedSizeCountList(
            IEnumerable<(Size Size, int Count)> overrideSizeCounts = null)
        {
            var expectedSizeCounts = Size.All.Select(c => new SizeCount() { Size = c, Count = 0 }).ToList();
            if (overrideSizeCounts != null)
            {
                foreach (var (size, count) in overrideSizeCounts)
                {
                    expectedSizeCounts.First(c => c.Size == size).Count = count;
                }
            }

            return expectedSizeCounts;
        }
    }
}
